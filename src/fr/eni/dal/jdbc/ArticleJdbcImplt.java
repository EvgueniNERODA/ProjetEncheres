...
public class ArticleJdbcImplt implements ArticleDAO {
	...
	private static final String SELECT_ALL_ARTICLES_BY_MOT_CLE = "SELECT nom_article, prix_initial, date_fin_encheres, pseudo, no_categorie, date_debut_encheres, description, prix_vente FROM ARTICLES_VENDUS AS a INNER JOIN UTILISATEURS AS u ON a.no_utilisateur=u.no_utilisateur  WHERE lower(nom_article) LIKE ? ";
	private static final String SELECT_ARTICLES_SELON_CATEGORIE_ET_MOT_CLE = "SELECT nom_article, prix_initial, date_fin_encheres, pseudo, no_categorie,date_debut_encheres, description, prix_vente FROM ARTICLES_VENDUS AS a INNER JOIN UTILISATEURS AS u ON a.no_utilisateur=u.no_utilisateur  WHERE lower(nom_article) LIKE ? AND no_categorie=? ";


	
	private static final String SELECT_ALL_ARTICLES = "SELECT nom_article, prix_initial, date_fin_encheres, pseudo, description, date_debut_encheres, prix_vente, date_debut_encheres, description, prix_vente   FROM ARTICLES_VENDUS JOIN UTILISATEURS ON ARTICLES_VENDUS.no_utilisateur=UTILISATEURS.no_utilisateur ";
			...
			while (rs.next()) { 
				...
				if (rs.getDate("date_debut_encheres").toLocalDate().isBefore((LocalDate.now())) || rs.getDate("date_debut_encheres").toLocalDate().isEqual(((LocalDate.now())))) {
					if (rs.getDate("date_fin_encheres").toLocalDate().isAfter(LocalDate.now())  || rs.getDate("date_fin_encheres").toLocalDate().isEqual(LocalDate.now())) {
						...
						listeDesArticles.add(new Article(rs.getString("nom_article"), rs.getString("description"), rs.getDate("date_debut_encheres").toLocalDate(),rs.getDate("date_fin_encheres").toLocalDate(), rs.getInt("prix_initial"), rs.getInt("prix_vente"), utilisateur ));
					}
